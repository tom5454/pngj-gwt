...
public class PngReader implements Closeable {
	 ...
	 */
	public final ImageInfo imgInfo; // People always told me: be careful what you do, and don't go around declaring
	// public
	// fields...
	/**
	 ...
	 * all) rows only if calling {@link #readRows()} or for interlaced images
	 ...
	 * used. See {@link ImageLineSetDefault} for examples
	 */
	 ...
	 * {@link #close()}d. See {@link #PngReader(InputStream,boolean)} or
	...
	public PngReader(InputStream inputStream, boolean shouldCloseStream) {
		...
		try {
			if (streamFeeder.feedFixed(chunkseq, 36) != 36) // 8+13+12=36 PNG signature+IHDR chunk
	...
	public IImageLine readRow(int nrow) {
		...
		if (!interlaced) {
			...
			while (rowNum < nrow) {
				...
				if (rowNum == nrow) {
					line.readFromPngRaw(chunkseq.getIdatSet().getUnfilteredRow(), getCurImgInfo().bytesPerRow + 1, 0,
							1);
	 ...
	 * reading the pixels. See {@link #getSimpleDigestHex()}
	 */
